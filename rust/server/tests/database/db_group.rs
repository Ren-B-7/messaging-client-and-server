...
async fn _insert_user(pool: &SqlitePool, user_id: i64, username: &str) {
...
async fn create_group_returns_valid_chat_id() {
    ...
    _insert_user(&pool, 1, "creator").await;
...
async fn creator_is_auto_added_as_admin_member() {
    ...
    let members = groups::get_group_members(&pool, chat_id).await.unwrap();
...
async fn add_member_increases_member_count() {
    let pool = setup_test_db().await;
    _insert_user(&pool, 1, "creator").await;
    _insert_user(&pool, 2, "member").await;
    let chat_id = groups::create_group(
        &pool,
        NewGroup {
            name: "G".into(),
    ...
    groups::add_group_member(&pool, chat_id, 2, "member".into())
        .await
        .unwrap();
...
async fn remove_member_decreases_member_count() {
    ...
    let removed = groups::remove_group_member(&pool, chat_id, 2)
        .await
        .unwrap();
...
async fn remove_nonexistent_member_returns_false() {
    ...
    let removed = groups::remove_group_member(&pool, chat_id, 999)
        .await
        .unwrap();
    assert!(!removed);
}
...
async fn is_group_member_true_for_creator() {
    let pool = setup_test_db().await;
    _insert_user(&pool, 7, "creator").await;
    let chat_id = groups::create_group(
        &pool,
        NewGroup {
            name: "G".into(),
    ...
    assert!(groups::is_group_member(&pool, chat_id, 7).await.unwrap());
}
...
async fn is_group_member_false_for_non_member() {
    ...
    assert!(!groups::is_group_member(&pool, chat_id, 99).await.unwrap());
}

#[tokio::test]
async fn is_group_admin_true_for_creator() {
    let pool = setup_test_db().await;
    _insert_user(&pool, 5, "creator").await;
    let chat_id = groups::create_group(
        &pool,
        NewGroup {
            name: "G".into(),
    ...
    assert!(groups::is_group_admin(&pool, chat_id, 5).await.unwrap());
}
...
async fn is_group_admin_false_for_regular_member() {
    ...
    assert!(!groups::is_group_admin(&pool, chat_id, 2).await.unwrap());
}
...
async fn delete_group_removes_all_related_rows() {
    ...
    // Delete the group
    groups::delete_group(&pool, chat_id).await.unwrap();

    // Group must be gone
    let group = groups::get_group(&pool, chat_id).await.unwrap();
    ...
    let history = messages::get_chat_messages(&pool, chat_id, 100, 0)
        .await
        .unwrap();
...
async fn find_existing_dm_returns_none_when_no_dm() {
    let pool = setup_test_db().await;
    let result = groups::find_existing_dm(&pool, 1, 2).await.unwrap();
...
async fn find_existing_dm_finds_shared_direct_chat() {
    let pool = setup_test_db().await;
    _insert_user(&pool, 1, "alice").await;
    _insert_user(&pool, 2, "bob").await;
    ...
    let found = groups::find_existing_dm(&pool, 1, 2).await.unwrap();
    assert_eq!(found, Some(chat_id));

    // Symmetric lookup
    let found_rev = groups::find_existing_dm(&pool, 2, 1).await.unwrap();
...
async fn groups_ordered_by_most_recent_message() {
    ...
    let groups_list = groups::get_user_groups_by_activity(&pool, 1).await.unwrap();
