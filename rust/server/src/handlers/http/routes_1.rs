...
type RouteHandler = Box<
    dyn Fn(Request<hyper::body::Incoming>, AppState) -> Pin<Box<dyn Future<Output = Result<Response<http_body_util::Full<Bytes>>>> + Send>>
        + Send
        + Sync,
>;
...
impl Router {
    ...
    where
        ...
        Fut: Future<Output = Result<Response<http_body_util::Full<Bytes>>>> + Send + 'static,
    {
    ...
    pub async fn route(
        ...
        for route in &self.routes {
            if route.method == method && Self::path_matches(&route.path, &path) {
                ...
                return Ok(convert_response_body(response));
            }
        }
    ...
    fn try_serve_static(
        ...
        let web_dir = self
            .web_dir
            .as_ref()
            .unwrap_or(&state.config.paths.web_dir);
        let icons = self
            .icons_dir
            .as_ref()
            .unwrap_or(&state.config.paths.icons);
...
pub fn build_user_router_with_config(web_dir: Option<String>, icons_dir: Option<String>) -> Router {
    ...
    router
        ...
        .post("/logout", |req, state| async move {
            info!("API: Processing logout");
            auth::handle_logout(req, state)
                .await
                .context("Logout failed")
