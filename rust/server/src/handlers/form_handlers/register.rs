...
/// TODO: Replace with proper password hashing (argon2, bcrypt, or scrypt)
fn hash_password(password: &str) -> Result<String> {
    // CRITICAL: DO NOT USE IN PRODUCTION
    // This is a placeholder. Use argon2, bcrypt, or scrypt
    use std::collections::hash_map::DefaultHasher;
    use std::hash::{Hash, Hasher};

    let mut hasher = DefaultHasher::new();
    password.hash(&mut hasher);
    Ok(format!("INSECURE_HASH_{}", hasher.finish()))
}
...
async fn attempt_registration(
    ...
    // Call: crate::database::register::create_user(...)
    ...
    if data.email.as_deref() == Some("taken@example.com") {
