...
const ChatMessages = {
  ...
  _currentUserId() { return ChatState.currentUser?.id ?? null; },
  ...
  _renderItem({id, text, content, timestamp, isSent, sender_id, read_at}) {
    ...
    return `
      <div class="message ${sentByMe ? 'sent' : 'received'}" data-msg-id="${
        id ?? ''}">
  ...
  async send() {
    ...
    try {
      ...
      const message = {
        ...
        content : text,
        timestamp :
            (messageData.sent_at || Math.floor(Date.now() / 1000)) * 1000,
        isSent : true,
      ...
      const conv =
          ChatState.findConversation(chatId) ?? ChatState.findGroup(chatId);
      if (conv) {
      ...
      this._showSendError(err.message ||
