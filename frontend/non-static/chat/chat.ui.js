...
const ChatUI = {
  ...
  updateHeader(conversation, type = "dm") {
    ...
    if (nameEl)
      nameEl.textContent = Utils.escapeHtml(conversation.name);

    // Render avatar: photo if available, initials otherwise.
    if (avatarEl) {
      if (type === "dm" && conversation.avatarUrl) {
        avatarEl.innerHTML = `<img src="${conversation.avatarUrl}" alt="${
            Utils.escapeHtml(conversation.name)}"
                style="width:100%;height:100%;object-fit:cover;border-radius:inherit"
                onerror="this.parentElement.textContent='${
            Utils.getInitials(conversation.name)}'">`;
      } else {
  ...
  _renderTyping() {
    ...
    banner.innerHTML = `
      ...
      <span class="typing-text">${
        count === 1 ? "Someone is typing"
                    : `${count} people are typing`}&hellip;</span>`;
  },
  ...
  setupActionButtons() {
    ...
    document.getElementById("addMemberSubmitBtn")
    ...
    document.querySelectorAll('[data-close-conv-modal="add-member-modal"]')
        .forEach((btn) => btn.addEventListener(
                     "click", () => this._closeAddMemberModal()));
    ...
    document.querySelectorAll('[data-close-conv-modal="group-settings-modal"]')
        .forEach((btn) => btn.addEventListener(
                     "click", () => this._closeModal("group-settings-modal")));
    ...
    // Rename
    document.getElementById("groupRenameBtn")
        ?.addEventListener("click", () => this._submitRename());
    document.getElementById("groupRenameInput")
        ?.addEventListener("keydown", (e) => {
          if (e.key === "Enter")
            this._submitRename();
        });
    ...
    document
        .querySelectorAll(
            '[data-close-conv-modal="group-delete-confirm-modal"]')
        .forEach(
            (btn) => btn.addEventListener(
                "click", () => this._closeModal("group-delete-confirm-modal")));
    document.getElementById("group-delete-confirm-modal")
        ?.addEventListener("click", (e) => {
          if (e.target === e.currentTarget)
            this._closeModal("group-delete-confirm-modal");
        });
    document.getElementById("groupDeleteConfirmBtn")
        ?.addEventListener("click", () => this._submitDeleteGroup());

    // Add member inside settings
    document.getElementById("groupAddMemberBtn")
        ?.addEventListener("click", () => this._submitAddMemberFromSettings());
    document.getElementById("groupAddMemberInput")
        ?.addEventListener("keydown", (e) => {
          if (e.key === "Enter")
            this._submitAddMemberFromSettings();
        });
    document.getElementById("groupAddMemberInput")
        ?.addEventListener(
            "input",
  ...
  async _loadMembers(chatId) {
    ...
    listEl.innerHTML =
        `<p style="color:var(--fg-tertiary);font-size:var(--text-sm)">Loading…</p>`;

    try {
      const res =
          await fetch(`/api/groups/${encodeURIComponent(chatId)}/members`);
      if (!res.ok)
      ...
      if (!members.length) {
        listEl.innerHTML =
            `<p style="color:var(--fg-tertiary);font-size:var(--text-sm)">No members found.</p>`;
        return;
      }
      ...
      listEl.innerHTML =
          ...
          <div class="group-member-row" data-user-id="${m.user_id}">
            <div class="avatar avatar-sm">${
                    Utils.getInitials(displayName)}</div>
            <div class="group-member-info">
              <span class="group-member-name">${
                    Utils.escapeHtml(displayName)}</span>
              ${
                    m.role && m.role !== 'member'
                        ? `<span class="group-member-role">${
                              Utils.escapeHtml(m.role)}</span>`
                        : ''}
      ...
      listEl.innerHTML =
          `<p style="color:var(--danger);font-size:var(--text-sm)">Failed to load members.</p>`;
  ...
  async _submitRename() {
    ...
    try {
      ...
      if (errEl)
        errEl.textContent = e.message || 'Failed to rename.';
  ...
  async _submitDeleteGroup() {
    ...
    try {
      ...
      if (errEl)
        errEl.textContent = e.message || 'Failed to delete group.';
  ...
  async _submitAddMemberFromSettings() {
    ...
    try {
      const res =
          await fetch(`/api/groups/${encodeURIComponent(conv.id)}/members`, {
            method : 'POST',
            headers : {'Content-Type' : 'application/json'},
      ...
      if (errEl)
        errEl.textContent = e.message || 'Failed to add member.';
  ...
  async _searchUsers(query) {
    ...
    try {
      ...
      resultsEl.innerHTML = users
                                .map((u) => `
        <div class="group-search-result-item" data-username="${
      ...
      resultsEl.querySelectorAll('.group-search-result-item')
          .forEach((item) => {
  ...
  async _submitAddMember() {
    ...
    if (!name) {
      if (errorEl)
        errorEl.textContent = "Please enter a name or username.";
      return;
    }
    if (!conv) {
      if (errorEl)
        errorEl.textContent = "No active group selected.";
      return;
    }
  ...
  async _uploadAvatar(file) {
    ...
    try {
      ...
      const data = await res.json();
      const avatarUrl =
          data.avatar_url || `/api/avatar/${ChatState.currentUser?.id}`;
