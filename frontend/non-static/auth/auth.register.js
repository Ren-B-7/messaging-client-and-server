...
const AuthRegister = {
  ...
  _checkUrlError() {
    ...
    if (error === 'username_taken') {
      AuthLogin.showError('username', 'This username is already taken. Please choose another.');
    } else if (error === 'email_taken') {
      AuthLogin.showError('regEmail', 'This email is already registered. Please sign in instead.');
  ...
  _handleServerError(data) {
    switch (data.code) {
      ...
      default:
        ...
        this._goToStep(0);
        AuthLogin.showError('regEmail', data.message ?? 'An unexpected error occurred.');
    }
  },
