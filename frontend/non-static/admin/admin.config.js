...
const CONFIG_SCHEMA = {
    ...
    "paths.blocked_paths": {
        ...
        description: "URL paths that the server refuses unconditionally. One entry per line.",
    },
};
...
const CONFIG_SECTIONS = {
    ...
    auth: {
        icon: "🔑",
        title: "Auth & Security",
        description: "JWT, CORS, IP binding, and session settings",
    },
    paths: {
        icon: "📁",
        title: "Paths",
        description: "Filesystem paths for web assets and uploads",
    },
};
...
const AdminConfig = {
    ...
    async saveAll() {
        ...
        const hasDangerous = Object.keys(this._dirty).some((k) => CONFIG_SCHEMA[k]?.dangerous);
        ...
        try {
            ...
            if (data.status === "success") {
                ...
                AdminUI.logAction(
                    "info",
                    `Config updated — ${Object.keys(this._dirty).join(", ")}`
                );
                this._dirty = {};
