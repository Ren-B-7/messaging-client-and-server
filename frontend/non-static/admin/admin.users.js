 ...
 *   id, username, email, created_at (unix secs), is_banned, ban_reason, is_admin
 *
 ...
 *                       latency_p99_ms,
 *                       rate_limiter: { total_ips, rate_limited, capacity, refill_rate } }
 */
...
const AdminUsers = {
  ...
  _row(u) {
    ...
    const adminBadge = u.is_admin
      ? '<span class="badge badge-amber" title="Admin">★</span>'
      : "";

    const primaryBtn = u.is_banned
    ...
    const promoteBtn = u.is_admin
      ? `<button class="btn btn-sm btn-secondary" onclick="AdminActions.submitDemote(${u.id})" title="Remove admin">Demote</button>`
      : `<button class="btn btn-sm btn-secondary" onclick="AdminActions.submitPromote(${u.id})" title="Make admin">Promote</button>`;

    return `
      <tr class="${u.is_banned ? "is-banned" : ""}">
        <td>
          <div class="user-cell">
            <div class="user-avatar">${Utils.getInitials(u.username || "?")}</div>
            <div>
              <div class="user-name">${Utils.escapeHtml(u.username || "Unknown")} ${adminBadge}</div>
  ...
  _renderMetrics(m) {
    ...
    if (metricsEl) {
      metricsEl.innerHTML = this._infoRows({
        ...
        "Requests / sec":
          m.requests_per_second != null
            ? m.requests_per_second.toFixed(2)
            : "—",
        Uptime: m.uptime_secs != null ? `${Math.floor(m.uptime_secs)}s` : "—",
        "Bytes sent":
          m.bytes_sent != null ? Utils.formatFileSize(m.bytes_sent) : "—",
        "Bytes received":
          m.bytes_received != null
    ...
    if (latencyEl) {
      latencyEl.innerHTML = this._infoRows({
        "Avg latency":
          m.latency_avg_ms != null ? `${m.latency_avg_ms.toFixed(2)} ms` : "—",
        "P50 latency":
          m.latency_p50_ms != null ? `${m.latency_p50_ms.toFixed(2)} ms` : "—",
        "P95 latency":
          m.latency_p95_ms != null ? `${m.latency_p95_ms.toFixed(2)} ms` : "—",
        "P99 latency":
          m.latency_p99_ms != null ? `${m.latency_p99_ms.toFixed(2)} ms` : "—",
      });
    }
