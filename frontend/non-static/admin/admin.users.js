...
const AdminUsers = {
      ...
      <tr class="${u.is_banned ? "is-banned" : ""}">
        <td>
          <div class="user-cell">
            ...
            <div>
              <div class="user-name">${Utils.escapeHtml(
                  u.username || "Unknown"
              )} ${adminBadge}</div>
    ...
    _renderMetrics(m) {
        ...
        if (metricsEl) {
            metricsEl.innerHTML = this._infoRows({
                ...
                "Error rate": m.error_rate_pct != null ? `${m.error_rate_pct.toFixed(2)}%` : "—",
                "Requests / sec":
                    m.requests_per_second != null ? m.requests_per_second.toFixed(2) : "—",
                Uptime: m.uptime_secs != null ? `${Math.floor(m.uptime_secs)}s` : "—",
                "Bytes sent": m.bytes_sent != null ? Utils.formatFileSize(m.bytes_sent) : "—",
                "Bytes received":
                    m.bytes_received != null ? Utils.formatFileSize(m.bytes_received) : "—",
        ...
        if (latencyEl) {
            latencyEl.innerHTML = this._infoRows({
                "Avg latency": m.latency_avg_ms != null ? `${m.latency_avg_ms.toFixed(2)} ms` : "—",
                "P50 latency": m.latency_p50_ms != null ? `${m.latency_p50_ms.toFixed(2)} ms` : "—",
                "P95 latency": m.latency_p95_ms != null ? `${m.latency_p95_ms.toFixed(2)} ms` : "—",
                "P99 latency": m.latency_p99_ms != null ? `${m.latency_p99_ms.toFixed(2)} ms` : "—",
            });
        }
    ...
    _renderSessionsTable(sessions) {
        ...
        if (!sessions.length) {
            tbody.innerHTML = `<tr><td colspan="5">${AdminUI.emptyHtml(
                "🔑",
                "No sessions."
            )}</td></tr>`;
            return;
            ...
            .map((s) => {
                ...
                const expiresCell = expired
                    ? `<span class="session-expired">${
                          expires
                      } <span class="badge badge-red">Expired</span></span>`
                    : expires;
                return `
