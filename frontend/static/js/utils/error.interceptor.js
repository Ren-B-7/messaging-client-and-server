 ...
 *     icon:     string            — Emoji icon (falls back to defaults per
 * code) primary:  { label, href? }  — Primary CTA button (defaults to "Go Home
 ...
 *
 * Drop-in notes:
 *   - Remove the <template id="__error_template__"> blocks from every page
 * HTML.
 *   - Remove the inline <script defer
 * src="static/js/utils/error.interceptor.js"> from the bottom of each page body
 * — keep it in <head> instead so global error listeners are registered as early
 * as possible.
    ...
    response.clone()
        ...
        .then(function(body) {
          ...
          redirect(Object.assign({}, base, {
            subtitle :
                body.message || body.error || body.detail || base.subtitle,
            hint : body.hint || body.description || base.hint,
            title : body.title || base.title,
          }));
        })
        .catch(function() { redirect(base); });
  } else {
    redirect(base);
  }
}
 ...
 *   - NetworkError on *.map requests     → source map fetch (DevTools opens map
 * files)
...
function isDevToolsNoise(event) {
  ...
  // Extension scripts
  if (/^(chrome|moz|safari)-extension:\/\//.test(filename))
    return true;
  // DevTools internal
  if (filename.startsWith("devtools://"))
    return true;
  // Source map fetch failures triggered when DevTools is open
  if (filename.endsWith(".map") || reasonMsg.includes(".map"))
    return true;
  // ResizeObserver loop limit — harmless browser internals
  if (msg.includes("resizeobserver loop"))
    return true;
  if (reasonMsg.includes("resizeobserver loop"))
    return true;
  // HMR / webpack hot reload noise
  if (msg.includes("hot") && msg.includes("undefined"))
    return true;
  ...
  if (!filename && event.lineno === 0 && event.colno === 0)
    return true;
