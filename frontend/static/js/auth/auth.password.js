...
const AuthPassword = {
  ...
  setupToggles() {
    document.querySelectorAll(".password-toggle").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (img)
          img.src = isHidden ? "static/icons/icons/eye-off.svg"
  ...
  _renderStrength(level, hints = []) {
    ...
    if (fill) {
      fill.className =
          level === "empty" ? "strength-fill" : `strength-fill ${level}`;
    }
    ...
    if (label) {
      ...
      label.textContent =
          level === "empty" ? "" : `Password strength: ${labels[level]}`;
    }
    ...
    if (hintEl) {
      if (hints.length === 0 || level === "empty") {
        ...
        hintEl.innerHTML =
            hints.map((h) => `<span class="strength-hint-item">✗ ${h}</span>`)
                .join("");
      }
    }
  },
